...
**Чуковский → Розенталь → Слопотрон → Мильчин** (смысл → буква → детектор → форма).
...
Аграновский — **отдельная ось**, не вписанная в этот конвейер. Его запускают по запросу,
когда текст содержит конкретные утверждения (числа, цитаты, законы, исследования) или когда
он написан с помощью LLM и доверие к фактуре под вопросом. Это дорогой инструмент —
десятки поисков на лонгрид. Применяйте целенаправленно.

Скил смыслового редактора (Чуковский) помечает подозрительные факты флагом, но не
проверяет их. Аграновский — следующий шаг: берёт эти флаги и верифицирует каждый
по первоисточнику.
